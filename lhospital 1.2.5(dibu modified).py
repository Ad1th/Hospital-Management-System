...
def admin_menu():
    ...
    view_patients_button = tk.Button(admin_frame, text="VIEW ALL PATIENTS", command=view_all_patients, font=("Arial", 16), bg=admin_bg)
    view_patients_button.pack(pady=20, padx=30, anchor='w')
...
def view_all_patients():
    ...
    # Display patient details in a separate window
    patient_window = tk.Tk()
    patient_window.title("Patient Details")

    details_frame = tk.Frame(patient_window)
    ...
    for row_num, patient in enumerate(patient_data, start=1):
        for col_num, detail in enumerate(patient):
...
def doctor_page(doctor_id):
    ...
    cursor.execute(
        "SELECT AppointmentID, PatientID, name, AppointmentDate, AppointmentTime, Age, gender, Bloodgroup, BriefNote, BookingTime "
        "FROM appointments NATURAL JOIN patientdetails "
        "WHERE DoctorNo = %s AND AppointmentDate >= %s "
    ...
    if doctor_data:
        ...
        labels = tk.Label(
            appt_frame,
            text="Appointment ID     Patient ID\t\tPatient Name \tAppointment Date\t    Appointment Time\t   Age\t     Gender \t Bloodgroup\t\tBrief Note \t\tBooking Time",
            font=("Arial", 10)
        )
        labels.grid(row=1, column=0, padx=5, pady=5)

        appointments_list = tk.Listbox(appt_frame, font=("Arial", 9, ), selectbackground="#A6A6A6", selectmode="extended",
                                       height=7, width=175)
